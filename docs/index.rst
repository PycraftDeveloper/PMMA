...
Let's walk you through PMMA's additional syntax now:
    ...
    2. Secondly, the icon after the deployment indicator (in bold) indicates if the namespace uses any additional optimization.
        * The **R** symbol - indicates that this namespace has no additional optimization (meaning **R**aw Python) - *no speed up*
        * The **N** symbol - indicates that this namespace *can* use Numba (JIT) compilation (meaning **N**umba) - *minor speed up*
        * The **C** symbol - indicates that this namespace *can* use Cython compilation (meaning **C**ython) - *major speed up*
