...
void CPP_Core2D_RenderPipelineInstance::Render() { // called once per frame
	ColorTexture.Assemble();

	if (bgfx::isValid(instanceVbh)) {
		if (instanceCount != PreviousInstanceCount) {
			bgfx::destroy(instanceVbh);
			instanceVbh = bgfx::createDynamicVertexBuffer(bgfx::copy(
				instanceDataArray.data(),
				instanceCount * sizeof(InstanceData)),
				instanceLayout);
		}
		else {
			bgfx::update(instanceVbh, 0, bgfx::copy(instanceDataArray.data(), instanceCount * sizeof(InstanceData)));
		}
	}
	else {
		instanceVbh = bgfx::createDynamicVertexBuffer(bgfx::copy(
