...
class EXPORT FractalBrownianMotion {
private:
    uint32_t Seed;
    uint32_t Octaves;
    float Lacunarity;
    ...
    PMMA::Noise::PerlinNoise *PerlinNoise_ptr = nullptr;

public:
    FractalBrownianMotion(const uint32_t new_seed, uint32_t new_octaves, float new_frequency, float new_amplitude);
    ...
    inline float Noise1D(const float x) const noexcept {
        ...
        for (uint32_t i = 0; i < Octaves; i++) {
            total += PerlinNoise_ptr->Noise1D(x * frequency) * amplitude;
    ...
    inline float Noise2D(const float x, const float y) const noexcept {
        ...
        for (uint32_t i = 0; i < Octaves; i++) {
            total += PerlinNoise_ptr->Noise2D(x * frequency, y * frequency) * amplitude;
            max_amplitude += amplitude;
            frequency *= Lacunarity;
            amplitude *= Gain;
        }
    ...
    inline float Noise3D(const float x, const float y, const float z) const noexcept {
        ...
        for (uint32_t i = 0; i < Octaves; i++) {
            total += PerlinNoise_ptr->Noise3D(x * frequency, y * frequency, z * frequency) * amplitude;
            max_amplitude += amplitude;
            frequency *= Lacunarity;
            amplitude *= Gain;
        }
