...
public:
    ...
    template <typename T>
    inline void AddRenderTarget(T *TargetPtr) {
        const bool ShapeVertexDataChanged = TargetPtr->VertexDataChanged;
        const bool ShapeColorDataChanged = TargetPtr->ColorDataChanged;
        VertexDataChanged |= ShapeVertexDataChanged;
        ColorDataChanged |= ShapeColorDataChanged;
        ...
        if (currentIndex < PreviousRenderContent[LivePreviousRenderContent].size()) {
            ...
            if (TargetPtr->ID == existingID && !ShapeVertexDataChanged && !ShapeColorDataChanged && PreviousFrameDataValid) {
