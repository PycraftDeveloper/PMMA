...
EXPORT inline void LookAt(
    ...
    InPlaceArrayNormalize3D(f);

    float upn[3] = {up[0], up[1], up[2]};
    InPlaceArrayNormalize3D(upn);

    float s[3];
    Cross(f, upn, s);
    InPlaceArrayNormalize3D(s);
...
EXPORT inline void ComputePosition(
    ...
    InPlaceArrayNormalize3D(out_z);

    Cross(up, out_z, out_x);
    InPlaceArrayNormalize3D(out_x);
...
EXPORT inline float RandomFloat(const float *out_range) {
    ...
    return Ranger(value, default_range, out_range);
}
