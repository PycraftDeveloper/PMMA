from typing import Literal, Union, Callable, Any, Generator

import numpy
from numpy.typing import NDArray
import pedalboard
from moviepy.audio.io.AudioFileClip import AudioFileClip
import sounddevice

Numerical = Union[float, int]

class Audio:
    def __init__(self) -> None: ...

    def __del__(self) -> None: ...

    def set_custom_audio_callback_function_pre_effects(self, function: Union[Callable[..., Any], None]=None) -> None: ...

    def set_custom_audio_callback_function_post_effects(self, function: Union[Callable[..., Any], None]=None) -> None: ...

    def get_custom_audio_callback_function_pre_effects_results(self) -> Any: ...

    def get_custom_audio_callback_function_post_effects_results(self) -> Any: ...

    def load_from_file(self, file_path: str) -> None: ...

    def get_sample_rate(self) -> int: ...

    def get_number_of_channels(self) -> int: ...

    def set_looping(self, loop: bool) -> None: ...

    def get_looping(self) -> bool: ...

    def set_duration(self, duration: Numerical) -> None: ...

    def set_silence_at_end_of_track(self, duration: Numerical) -> None: ...

    def load_from_moviepy(self, audio: AudioFileClip) -> None: ...

    def add_effect(
            self,
            effect: Union[BitCrush, Chorus, Clipping, Compressor,
                            Convolution, Delay, Distortion,
                            GSMFullRateCompressor, Gain,
                            HighShelfFilter, HighPassFilter,
                            LadderFilter, Limiter, LowShelfFilter,
                            LowPassFilter, MP3Compressor, NoiseGate,
                            PeakFilter, Phaser, PitchShift, ReSample,
                            Reverb]) -> None: ...

    def remove_effect(
            self,
            effect: Union[BitCrush, Chorus, Clipping, Compressor,
                            Convolution, Delay, Distortion,
                            GSMFullRateCompressor, Gain,
                            HighShelfFilter, HighPassFilter,
                            LadderFilter, Limiter, LowShelfFilter,
                            LowPassFilter, MP3Compressor, NoiseGate,
                            PeakFilter, Phaser, PitchShift, ReSample,
                            Reverb]) -> None: ...

    def set_volume(self, volume: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def set_pan(self, pan: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def play(self, blocking: bool=True, delay: Numerical=0) -> bool: ...

    def _wait_for_chunk_to_play(self) -> bool: ...

    def _start_playback(self) -> None: ...

    def _audio_generator(self, chunk_size: int) -> Generator[NDArray[numpy.float32], None, None]: ...

    def _audio_callback(self, outdata: NDArray[numpy.float32], frames: int, _: Numerical, status: sounddevice.CallbackFlags) -> None: ...

    def _apply_volume_and_pan(self, chunk: NDArray[numpy.float32]) -> NDArray[numpy.float32]: ...

    def pause(self) -> None: ...

    def resume(self) -> None: ...

    def stop(self) -> None: ...

    def get_paused(self) -> bool: ...

    def get_playing(self) -> bool: ...

class BitCrush(pedalboard.Bitcrush):
    def __init__(self, bit_depth: int=8) -> None: ...

    def set_bit_depth(self, bit_depth: int) -> None: ...

    def get_bit_depth(self) -> int: ...

class Chorus(pedalboard.Chorus):
    def __init__(
            self,
            rate: int=1,
            depth: Numerical=25,
            center_delay: Numerical=0.007,
            feedback: Numerical=0,
            mix: Numerical=50,
            format: Literal["percentage", "decimal"]="percentage",
            depth_format: Literal["percentage", "decimal", None]=None,
            feedback_format: Literal["percentage", "decimal", None]=None,
            mix_format: Literal["percentage", "decimal", None]=None) -> None: ...

    def set_rate(self, rate: int) -> None: ...

    def get_rate(self) -> int: ...

    def set_depth(self, depth: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_depth(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_center_delay(self, center_delay: Numerical) -> None: ...

    def get_center_delay(self) -> Numerical: ...

    def set_feedback(self, feedback: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_feedback(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_mix(self, mix: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_mix(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

class Clipping(pedalboard.Clipping):
    def __init__(self, threshold: Numerical=-6) -> None: ...

    def set_threshold(self, threshold: Numerical) -> None: ...

    def get_threshold(self) -> Numerical: ...

class Compressor(pedalboard.Compressor):
    def __init__(
            self,
            threshold: Numerical=0,
            ratio: Numerical=4,
            attack: Numerical=0.001,
            release: Numerical=0.1) -> None: ...

    def set_threshold(self, threshold: Numerical) -> None: ...

    def get_threshold(self) -> Numerical: ...

    def set_ratio(self, ratio: Numerical) -> None: ...

    def get_ratio(self) -> Numerical: ...

    def set_attack(self, attack: Numerical) -> None: ...

    def get_attack(self) -> Numerical: ...

    def set_release(self, release: Numerical) -> None: ...

    def get_release(self) -> Numerical: ...

class Convolution(pedalboard.Convolution):
    def __init__(
            self,
            impulse_response_filename: str,
            mix: Numerical=100,
            sample_rate: Union[int, None]=None,
            format: Literal["percentage", "decimal"]="percentage",
            mix_format: Literal["percentage", "decimal", None]=None) -> None: ...

    def set_impulse_response_filename(self, impulse_response: str) -> None: ...

    def get_impulse_response_filename(self) -> str: ...

    def set_mix(self, mix: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_mix(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_sample_rate(self, sample_rate: Union[int, None]) -> None: ...

    def get_sample_rate(self) -> Union[int, None]: ...

class Delay(pedalboard.Delay):
    def __init__(
            self,
            delay: Numerical=0.5,
            feedback: Numerical=0,
            mix: Numerical=50,
            format: Literal["percentage", "decimal"]="percentage",
            feedback_format: Literal["percentage", "decimal", None]=None,
            mix_format: Literal["percentage", "decimal", None]=None) -> None: ...

    def set_delay(self, delay: Numerical) -> None: ...

    def get_delay(self) -> Numerical: ...

    def set_feedback(self, feedback: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_feedback(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_mix(self, mix: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_mix(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

class Distortion(pedalboard.Distortion):
    def __init__(self, drive: Numerical=10) -> None: ...

    def set_drive(self, drive: Numerical) -> None: ...

    def get_drive(self) -> Numerical: ...

class GSMFullRateCompressor(pedalboard.GSMFullRateCompressor):
    def __init__(self) -> None: ...

class Gain(pedalboard.Gain):
    def __init__(self, gain: Numerical=1) -> None: ...

    def set_gain(self, gain: Numerical) -> None: ...

    def get_gain(self) -> Numerical: ...

class HighShelfFilter(pedalboard.HighShelfFilter):
    def __init__(self, cutoff: int=440, gain: Numerical=0, q: Numerical=0.7071067690849304) -> None: ...

    def set_cutoff(self, cutoff: int) -> None: ...

    def get_cutoff(self) -> int: ...

    def set_gain(self, gain: Numerical) -> None: ...

    def get_gain(self) -> Numerical: ...

    def set_q(self, q: Numerical) -> None: ...

    def get_q(self) -> Numerical: ...

class HighPassFilter(pedalboard.HighpassFilter):
    def __init__(self, cutoff: int=50) -> None: ...

    def set_cutoff(self, cutoff: int) -> None: ...

    def get_cutoff(self) -> int: ...

class LadderFilter(pedalboard.LadderFilter):
    def __init__(self, cutoff: int=200, resonance: Numerical=0, drive: Numerical=1) -> None: ...

    def set_cutoff(self, cutoff: int) -> None: ...

    def get_cutoff(self) -> int: ...

    def set_resonance(self, resonance: Numerical) -> None: ...

    def get_resonance(self) -> Numerical: ...

    def set_drive(self, drive: Numerical) -> None: ...

    def get_drive(self) -> Numerical: ...

class Limiter(pedalboard.Limiter):
    def __init__(self, threshold: Numerical=-10, release: Numerical=0.1) -> None: ...

    def set_threshold(self, threshold: Numerical) -> None: ...

    def get_threshold(self) -> Numerical: ...

    def set_release(self, release: Numerical) -> None: ...

    def get_release(self) -> Numerical: ...

class LowShelfFilter(pedalboard.LowShelfFilter):
    def __init__(self, cutoff: int=440, gain: Numerical=0, q: Numerical=0.7071067690849304) -> None: ...

    def set_cutoff(self, cutoff: int) -> None: ...

    def get_cutoff(self) -> int: ...

    def set_gain(self, gain: Numerical) -> None: ...

    def get_gain(self) -> Numerical: ...

    def set_q(self, q: Numerical) -> None: ...

    def get_q(self) -> Numerical: ...

class LowPassFilter(pedalboard.LowpassFilter):
    def __init__(self, cutoff: int=50) -> None: ...

    def set_cutoff(self, cutoff: int) -> None: ...

    def get_cutoff(self) -> int: ...

class MP3Compressor(pedalboard.MP3Compressor):
    def __init__(self, vbr_quality: Numerical=3) -> None: ...

    def set_vbr_quality(self, vbr_quality: Numerical) -> None: ...

    def get_vbr_quality(self) -> Numerical: ...

class NoiseGate(pedalboard.NoiseGate):
    def __init__(
            self,
            threshold: Numerical=-100,
            ratio: Numerical=10,
            attack: Numerical=0.001,
            release: Numerical=0.1) -> None: ...

    def set_threshold(self, threshold: Numerical) -> None: ...

    def get_threshold(self) -> Numerical: ...

    def set_ratio(self, ratio: Numerical) -> None: ...

    def get_ratio(self) -> Numerical: ...

    def set_attack(self, attack: Numerical) -> None: ...

    def get_attack(self) -> Numerical: ...

    def set_release(self, release: Numerical) -> None: ...

    def get_release(self) -> Numerical: ...

class PeakFilter(pedalboard.PeakFilter):
    def __init__(self, frequency: int=1000, gain: float=0, q: float=0.7071067690849304) -> None: ...

    def set_frequency(self, frequency: int) -> None: ...

    def get_frequency(self) -> int: ...

    def set_gain(self, gain: float) -> None: ...

    def get_gain(self) -> float: ...

    def set_q(self, q: float) -> None: ...

    def get_q(self) -> float: ...

class Phaser(pedalboard.Phaser):
    def __init__(
            self,
            rate: int=1,
            depth: Numerical=50,
            center_frequency: int=1300,
            feedback: Numerical=0,
            mix: Numerical=50,
            format: Literal["percentage", "decimal"]="percentage",
            depth_format: Literal["percentage", "decimal", None]=None,
            feedback_format: Literal["percentage", "decimal", None]=None,
            mix_format: Literal["percentage", "decimal", None]=None) -> None: ...

    def set_rate(self, rate: int) -> None: ...

    def get_rate(self) -> int: ...

    def set_depth(self, depth: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_depth(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_center_frequency(self, center_frequency: int) -> None: ...

    def get_center_frequency(self) -> int: ...

    def set_feedback(self, feedback: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_feedback(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_mix(self, mix: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_mix(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

class PitchShift(pedalboard.PitchShift):
    def __init__(self, semitones: Numerical=0) -> None: ...

    def set_semitones(self, pitch_shift_semitones: Numerical) -> None: ...

    def get_semitones(self) -> Numerical: ...

class ReSample(pedalboard.Resample):
    def __init__(self, sample_rate: int=8000) -> None: ...

    def set_sample_rate(self, sample_rate: int) -> None: ...

    def get_sample_rate(self) -> int: ...

class Reverb(pedalboard.Reverb):
    def __init__(
            self,
            room_size: Numerical=100,
            damping: Numerical=50,
            wet_level: Numerical=33,
            dry_level: Numerical=40,
            width: Numerical=100,
            freeze_mode: bool=False,
            format: Literal["percentage", "decimal"]="percentage",
            room_size_format: Literal["percentage", "decimal", None]=None,
            damping_format: Literal["percentage", "decimal", None]=None,
            wet_level_format: Literal["percentage", "decimal", None]=None,
            dry_level_format: Literal["percentage", "decimal", None]=None,
            width_format: Literal["percentage", "decimal", None]=None) -> None: ...

    def set_room_size(self, room_size: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_room_size(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_damping(self, damping: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_damping(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_wet_level(self, wet_level: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_wet_level(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_dry_level(self, dry_level: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_dry_level(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_width(self, width: Numerical, format: Literal["percentage", "decimal"]="percentage") -> None: ...

    def get_width(self, format: Literal["percentage", "decimal"]="percentage") -> Numerical: ...

    def set_freeze_mode(self, freeze_mode: bool) -> None: ...

    def get_freeze_mode(self) -> bool: ...