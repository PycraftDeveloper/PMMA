...
def setup():
    try:
        _os.mkdir(f"{_Registry.base_path}{_os.sep}bin")
...
def compile_libraries():
    try:
        ...
        try:
            exit_code = _subprocess.call([_sys.executable, f"{_Registry.base_path}{_Constants.PATH_SEPARATOR}c_setup.py", "build_ext", "--build-lib", f"{_Registry.base_path}{_Constants.PATH_SEPARATOR}bin", "--build-temp", "temporary"])
...
def compile():
    thread = _threading.Thread(target=compile_intermediary)
