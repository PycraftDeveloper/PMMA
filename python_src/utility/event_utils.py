class Backspace_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Tab_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Clear_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Return_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Pause_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Escape_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Space_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class ExclamationMark_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class DoubleQuote_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Hashtag_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Dollar_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Ampersand_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class SingleQuote_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftParenthesis_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightParenthesis_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Asterisk_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Plus_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Comma_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Minus_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Period_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class ForwardSlash_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary0_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary1_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary2_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary3_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary4_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary5_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary6_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary7_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary8_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Primary9_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Colon_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class SemiColon_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LessThan_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Equals_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class GreaterThan_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class QuestionMark_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class At_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftBracket_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class BackSlash_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightBracket_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Caret_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Underscore_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Grave_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryA_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryB_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryC_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryD_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryE_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryF_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryG_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryH_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryI_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryJ_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryK_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryL_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryM_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryN_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryO_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryP_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryQ_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class primaryR_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryS_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryT_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryU_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryV_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryW_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryX_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryY_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PrimaryZ_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Delete_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad0_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad1_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad2_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad3_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad4_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad5_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad6_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad7_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad8_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Numpad9_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadPeriod_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadDivide_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadMultiply_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadMinus_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadPlus_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadEnter_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumpadEquals_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Up_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Down_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Right_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Left_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Insert_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Home_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class End_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PageUp_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class PageDown_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function1_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function2_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function3_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function4_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function5_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function6_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function7_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function8_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function9_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function10_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function11_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function12_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function13_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function14_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Function15_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class NumLock_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class CapsLock_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class ScrollLock_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightShift_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftShift_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Shift_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightControl_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftControl_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Control_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightAlt_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftAlt_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Alt_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightMeta_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftMeta_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Meta_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LeftSuper_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RightSuper_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Super_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Mode_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Help_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Print_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class SystemRequest_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Break_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Menu_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Power_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Euro_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class AndroidBack_KEY:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class Y_BUTTON: # 3
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class B_BUTTON: # 2
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class A_BUTTON: # 1
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class X_BUTTON: # 0
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class HOME_BUTTON: # 12
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RIGHT_JOYSTICK_BUTTON: # 11
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LEFT_JOYSTICK_BUTTON: # 10
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class OPTIONS_BUTTON: # 9
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class SHARE_BUTTON: # 8
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RIGHT_TRIGGER: # 7
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LEFT_TRIGGER: # 6
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RIGHT_BUMPER: # 5
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LEFT_BUMPER: # 4
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class CENTER_BUTTON: # 13
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class LEFT_JOYSTICK_AXIS:
    def __init__(self):
        self.x_axis = 0
        self.y_axis = 0

    def get_x_axis(self):
        return self.x_axis

    def get_y_axis(self):
        return self.y_axis

    def set_x_axis(self, value):
        self.x_axis = value

    def set_y_axis(self, value):
        self.y_axis = value

class RIGHT_JOYSTICK_AXIS:
    def __init__(self):
        self.x_axis = 0
        self.y_axis = 0

    def get_x_axis(self):
        return self.x_axis

    def get_y_axis(self):
        return self.y_axis

    def set_x_axis(self, value):
        self.x_axis = value

    def set_y_axis(self, value):
        self.y_axis = value

class LEFT_BUTTON_MOUSE:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class MIDDLE_BUTTON_MOUSE:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class RIGHT_BUTTON_MOUSE:
    def __init__(self):
        self._currently_pressed = False
        self._double_tap_timing = 0.25

    def get_pressed(self):
        return self._currently_pressed

    def set_pressed(self, value):
        self._currently_pressed = value

    def get_double_tap_timing(self):
        return self._double_tap_timing

    def set_double_tap_timing(self, value):
        self._double_tap_timing = value

class MOUSE_SCROLL:
    def __init__(self):
        self.scroll_value = 0
        self.scroll_displacement = 0

    def get_scroll_displacement(self):
        return self.scroll_displacement

    def get_scroll_value(self):
        return self.scroll_value

    def set_scroll_value(self, value):
        self.scroll_value = value

class MOUSE_POSITION:
    def __init__(self):
        self.x_axis = 0
        self.y_axis = 0
        self.x_axis_displacement = 0
        self.y_axis_displacement = 0

    def get_axis_displacement(self):
        return self.x_axis_displacement, self.y_axis_displacement

    def get_x_axis_displacement(self):
        return self.x_axis_displacement

    def get_y_axis_displacement(self):
        return self.y_axis_displacement

    def get_x_axis(self):
        return self.x_axis

    def get_y_axis(self):
        return self.y_axis

    def set_x_axis(self, value):
        self.x_axis = value

    def set_y_axis(self, value):
        self.y_axis = value