...
class Image:
    ...
    def create_from_file(self, image_path):
        if self.pil_image_address is not None:
            self.memory_manager_instance.remove(
                self.pil_image_address)

        start = _time.perf_counter()
        pil_image = _ImageModule.open(image_path)
        end = _time.perf_counter()
        self.pil_image_address = self.memory_manager_instance.add(
            pil_image,
    ...
    def image_to_PIL_object(self):
        return self.memory_manager_instance.get(
            self.pil_image_address)

    def image_to_display_renderable_object(self, auto_optimize=True):
        start = _time.perf_counter()
        pil_image = self.memory_manager_instance.get(
            self.pil_image_address)
        ...
        self.graphics_backend_image_address = self.memory_manager_instance.add(
    ...
    def blit(self, position, surface=None):
        ...
        if self.memory_manager_instance.get(self.graphics_backend_image_address) is None:
            object = self.image_to_display_renderable_object()
        else:

            object = self.memory_manager_instance.get(
